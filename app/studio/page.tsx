...
function StudioInner() {
  ...
  const initialSlug = search.get("product") || "blank-canvas-tee";
  const customizable = useMemo(
    () => products.filter((p) => p.customizable),
    []
  );
  const initial = getProduct(initialSlug) || customizable[0];
  ...
  const customPrice = product.price + (imageDataUrl ? 200 : 0);
    ...
    <div className="mx-auto max-w-7xl px-5 py-10 lg:px-8 lg:py-14">
      ...
      <div className="grid gap-6 lg:grid-cols-12">
        ...
        <aside className="order-3 lg:col-span-3">
          ...
          <button onClick={handleAddToCart} className="btn-primary mt-4 w-full" disabled={adding}>
            {adding ? "Adding to cart…" : `Add to cart — ${formatINR(customPrice * qty)}`}
          ...
          <p className="mt-3 px-1 text-[11px] leading-relaxed text-ink-400">
            ...
            Image uploads are stored locally on your device until you place
            the order. <Link href="/about" className="text-signal-lime">Print specs →</Link>
          </p>
        </aside>
      </div>
    </div>
